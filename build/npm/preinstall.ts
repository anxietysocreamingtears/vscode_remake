...
function installHeaders() {
	...
	// to that executable
	const nodeGypCli = path.join(import.meta.dirname, 'gyp', 'node_modules', 'node-gyp', 'bin', 'node-gyp.js');
	...
	if (local !== undefined) {
		...
		child_process.execFileSync(process.execPath, [nodeGypCli, 'install', '--dist-url', local.disturl, local.target], { stdio: 'inherit' });
	}

	if (remote !== undefined) {
		// Both disturl and target come from a file checked into our repository
		child_process.execFileSync(process.execPath, [nodeGypCli, 'install', '--dist-url', remote.disturl, remote.target], { stdio: 'inherit' });
	}
